...
document.getElementById("btnAgregarAlCarrito").addEventListener("click", () => {
    const idCarrito = document.getElementById("comboCarrito").value;
    const idProducto = document.getElementById("comboProductos").value;
    if (idCarrito && idProducto) {
        agregarCarrito(idCarrito, idProducto)
    } else {
        alert("Error: debe seleccionar un carrito y un producto")
    }
})

document.getElementById("btnCrearCarrito").addEventListener("click", () => {
    carritoApi.crearCarrito()
        .then(({ id }) => {
            loadComboCarrito().then(() => {
                const combo = document.getElementById("comboCarrito")
                combo.value = `${id}`
    ...
    if (productos.length > 0) {
        ...
        for (const prod of productos) {
            html += `
                    ...
                    <td>${prod.title}</td>
                    <td>$${prod.price}</td>
                    <td><img width="50" src=${prod.thumbnail} alt="picture"></td>
        ...
        html += `<br><h2>carrito sin productos</h2>`
    }
